...
class DeckContainer(Container):
    def __init__(self, master, deck_id, deck_name, card_count, selection_callback, avg_ef):
        ...
        # In the __init__ method where the checkbox is created
        self.checkbox = ctk.CTkCheckBox(
            ...
            hover_color=None  # Remove hover color
        )
        self.checkbox.place(relx=1.0, rely=0.0, anchor="ne", x=-10, y=10)

        # Bind the click event on container and its child widgets
        self.bind_all_children(self, "<Button-1>", self.toggle_selection)

    def bind_all_children(self, widget, event, callback):
        """Recursively bind an event to all child widgets to ensure container clicks work uniformly."""
    ...
    def on_checkbox_toggle(self):
        # Update selection state from the checkbox
        ...
        if self.selected:
            ...
            self.checkbox.configure(
                fg_color="#636ae8",  # Use the specified color when toggled
                checkmark_color="white"  # Change to white for better contrast
            )
        else:
...
class CardsPage(BasePage):
    def __init__(self, master, user_id, deck_id, session, switch_page):
...
class QuizPage(BasePage):
    ...
    def start_quiz(self):
        ...
        QuizSession(self.master, self.user_id, self.selected_deck, [self.selected_deck], self.session, self.switch_page)
