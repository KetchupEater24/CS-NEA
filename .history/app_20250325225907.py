...
class EditCardDialog(BaseDialog):
    ...
    def __init__(self, parent, card_id, current_question, current_answer):
        ...
        # creates text box for editing the question and pre-populates it with current question
        ...
        # creates text box for editing the answer and pre-populates it with current answer
        ...
        # (optionally, creates multimedia upload/preview sections here)

        # creates save changes button that calls self.save_card when clicked
        self.create_button("Save Changes", self.save_card)

        # waits until the dialog is closed before returning control to the parent
        self.wait_window()

    # saves the updated card information and closes the dialog
    def save_card(self):
        #
        ...
        try:
            ...
            if hasattr(self.parent, 'update_card_containers'):
                self.parent.update_card_containers()
            self.cancel_event()  # releases grab and closes the dialog
        except Exception as e:
...
class EditDeckDialog(BaseDialog):
    ...
    # saves the updated deck name and closes the dialog
    def save_deck(self):
        new_deck_name = self.deck_entry.get("1.0", "end-1c").strip()
        ...
        try:
            ...
            self.cancel_event()  # closes the dialog
        except Exception as e:
...
class AddDeckDialog(BaseDialog):
    ...
    # saves the new deck name and closes the dialog
    def save_deck(self):
        new_deck = self.deck_entry.get("1.0", "end-1c").strip()
        if not new_deck:
            messagebox.showwarning("Warning", "Please enter a deck name")
            return
        self.cancel_event()
...
class QuizPage(BasePage):
    ...
    def update_deck_containers(self):
        ...
        # create deck_containers with average EF, card count, etc.
        for deck_id, deck_name in decks:
            cards = db.get_cards(deck_id)
            if cards:
                total_ef = sum(db.get_card_easiness(self.user_id, c[0]) for c in cards)
