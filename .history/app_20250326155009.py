...
class DecksPage(BasePage):
    def __init__(self, master, user_id, switch_page):
        ...
        self.deck_search_var.trace_add("write", lambda *args: self.update_deck_list())
        self.deck_priority_filter_var.trace_add("write", lambda *args: self.update_deck_list())

        # Buttons container
        buttons_frame = ctk.CTkFrame(header_frame, fg_color="transparent")
        buttons_frame.pack(side="right")
        ctk.CTkButton(buttons_frame, text="+ Add", width=70, height=32, corner_radius=16,
                      fg_color="#F3F4F6", text_color="black", hover_color="#E5E7EB",
                      command=self.add_deck).pack(side="left", padx=5)
        self.delete_button = ctk.CTkButton(buttons_frame, text="Delete Selected", width=70, height=32,
    ...
    def update_deck_list(self):
        ...
        for node in sorted_nodes:
            deck_container = DeckContainer(self.decks_frame, deck_id=node.deck_id,
                                             deck_name=node.deck_name, card_count=node.card_count, user_id=node.user_id
...
class DeckContainer(BaseContainer):
    def __init__(self, master, deck_id, deck_name, card_count, avg_ef, edit_callback, delete_callback, user_id):
        ...
        self.user_id = user_id  # required for fetching available for review count

        # main content container: holds all the inner widgets
        self.content = ctk.CTkFrame(self, fg_color="transparent")
        self.content.pack(fill="both", expand=True, padx=20, pady=20)

        # left: deck info container that displays deck details
        ...
        # card count label: displays the number of cards in the deck
        ctk.CTkLabel(
            self.info_frame,
        ...
        # available for review label: shows how many cards are available for review
        db = Database()
        ...
        # priority indicator: shows the deck's priority based on the avg_ef value
        if self.avg_ef < 2.0:
        ...
        # buttons container: holds the edit and delete buttons (only if callbacks are provided)
        if self.edit_callback is not None and self.delete_callback is not None:
            ...
            # edit button: calls the edit callback when clicked
            ctk.CTkButton(
                btn_container,
                text="Edit",
                width=70,
            ...
            # delete button: calls the delete callback when clicked
            ctk.CTkButton(
                btn_container,
                text="Delete",
                width=70,
        ...
        # single-selection checkbox: allows the user to visually mark the deck as selected
    ...
    # called when the checkbox is toggled; updates the internal selection state and visual appearance
...
ontainer(BaseContainer):
    ...
    def __init__(self, master, card_id, question, answer,
        ...
        buttons_frame = ctk.CTkFrame(self.content, fg_color="transparent")
