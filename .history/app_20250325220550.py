...
class QuizPage(BasePage):
    def __init__(self, master, user_id, switch_page):
        ...
        # creates main header container with "Quiz" title
        self.header = ctk.CTkFrame(self.main_content, fg_color="transparent")
        self.header.pack(fill="x", padx=30, pady=(20, 0))
        ctk.CTkLabel(self.header, text="Quiz", font=("Inter", 24, "bold"), text_color="black").pack(side="left")
        ...
        # creates horizontal line between header and deck area
        self.separator = ctk.CTkFrame(self.main_content, height=1, fg_color="#E5E7EB")
        self.separator.pack(fill="x", padx=30, pady=(20, 0))

        # creates container for decks
        ...
        # creates label above decks list
        ctk.CTkLabel(
        ...
        # creates scrollable frame for deck containers
        self.decks_frame = ctk.CTkScrollableFrame(
            self.selection_frame,
            fg_color="transparent",
            scrollbar_button_color="#E5E7EB",
    ...
    def update_deck_containers(self):
        ...
        # creates a list of decks with ef and card count
        for deck_id, deck_name in decks:
        ...
        # filters decks by search
        search_query = self.deck_search_input.get().lower().strip()
        if search_query:
            deck_containers = [d for d in deck_containers if search_query in d[1].lower()]

        # filters decks by priority
        priority_filter = self.deck_priority_input.get().lower()
        if priority_filter != "all":
            if priority_filter == "high":
                deck_containers = [d for d in deck_containers if d[2] < 2.0]
        ...
        for node in sorted_nodes:
            deck_container = DeckContainer(
                ...
                selection_callback=self.toggle_deck
            )
            deck_container.grid(row=row, column=col, padx=10, pady=10, sticky="nsew")
    ...
    # toggles selected deck - only one deck allowed at a time
    def toggle_deck(self, deck_id, is_selected):
        if is_selected:
            for other_deck_id in list(self.selected_decks):
                if other_deck_id != deck_id:
                    other_container = self.deck_containers.get(other_deck_id)
                    if other_container:
                        other_container.selected = False
                        other_container.checkbox.deselect()
                        other_container.configure(fg_color="white")
                    self.selected_decks.discard(other_deck_id)
            self.selected_decks.add(deck_id)
        else:
            self.selected_decks.discard(deck_id)
    ...
    def start_quiz(self):
        if not self.selected_decks:
            ...
            return
        selected_deck = next(iter(self.selected_decks))
        for widget in self.master.winfo_children():
            widget.destroy()

        QuizSession(self.master, self.user_id, selected_deck, self.switch_page)
