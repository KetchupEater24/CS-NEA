...
class DeckContainer(BaseContainer):
    ...
    def __init__(self, master, deck_id, deck_name, card_count, avg_ef, edit_callback, delete_callback):
        ...
        # BUILDS A main deck container with fixed width to ensure equal sizing across three per row
        self.deck_container = ctk.CTkFrame(self, fg_color="transparent")
        # Set a fixed width (adjust as needed) and prevent auto-resizing to preserve button visibility
        self.deck_container.configure(width=300, height=100)
        self.deck_container.pack(fill="both", expand=True, padx=20, pady=20)
        self.deck_container.pack_propagate(False)

        # BUILDS A left container for deck info (name, card count, etc.)
        ...
        # get and display cards available for review count
        self.db = Database()
        # Previously, selection_callback.__self__ was used to obtain user_id.
        # That approach has been removed since multi-select tracking is used elsewhere.
        # Instead, pass user_id directly if needed (here we use None as placeholder).
        self.available_for_review = self.db.get_available_for_review(None, deck_id)
        ctk.CTkLabel(
            self.info,
        ...
        # BUILDS A priority indicator (based on avg_ef)
        if self.avg_ef < 2.0:
        ...
        # BUILDS A buttons container (Edit/Delete) at the bottom (only if callbacks provided)
        ...
        # BUILDS A multi-selection checkbox at the top right
    ...
    # callback for checkbox toggle to update selection and appearance
