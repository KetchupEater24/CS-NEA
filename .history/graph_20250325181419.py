...
class DeckNode:
    # initialises deck node with deck id, deck name, avg ef, card coutn, left pointer and right pointer
...
# performs in order traversal of the BST, returning nodes sorted by avg_ef (lowest first, e.g higher deck priority) 
def in_order(root):
    nodes = []
    if root:
