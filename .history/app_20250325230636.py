...
class DeckContainer(BaseContainer):
    def __init__(self, master, user_id, deck_id, deck_name, card_count, avg_ef, edit_callback, delete_callback):
        """
        Initializes a deck container with deck information, edit/delete buttons,
        and a checkbox for selection.
        
        Parameters:
            master (widget): The parent widget in which this container is placed.
            user_id (int): The user ID for querying deck-related data.
            deck_id (int): Unique identifier for the deck.
            deck_name (str): The name of the deck.
            card_count (int): Number of cards in the deck.
            avg_ef (float): Average easiness factor for the deck.
            edit_callback (function): Function to call when editing the deck.
            delete_callback (function): Function to call when deleting the deck.
        """
        ...
        # Main content container for deck info and buttons
        self.content = ctk.CTkFrame(self, fg_color="transparent")
        self.content.pack(fill="both", expand=True, padx=20, pady=20)

        # Left frame: deck information display (deck name, card count, review count, priority)
        ...
        # Display available for review count
        db = Database()
        ...
        # Priority indicator based on average easiness factor (avg_ef)
        if self.avg_ef < 2.0:
        ...
        # Buttons container for Edit and Delete actions (if callbacks are provided)
        ...
        # Single‐selection checkbox at TOP RIGHT to toggle selection state
    ...
    def toggle_selection(self, event=None):
        """
        Toggles the selection state of the deck container when a click event occurs
        (except when the click is directly on the checkbox).
        """
        # If the event originates from the checkbox, let its own handler manage the state.
        if event and event.widget == self.checkbox:
            return
        
        # Toggle the selection state and update the checkbox accordingly.
        self.selected = not self.selected
        if self.selected:
            self.checkbox.select()
        else:
            self.checkbox.deselect()
        # Update the visual background based on selection state.
        self.configure(fg_color="#F5F3FF" if self.selected else "white")

    def on_checkbox_toggle(self):
        """
        Handles the checkbox toggle event to update selection state and visual styling.
        """
        self.selected = self.checkbox.get()
        # Update the visual background and checkbox colors based on selection.
        if self.selected:
