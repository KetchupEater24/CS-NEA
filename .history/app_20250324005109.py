...
class AnalyticsPage(BasePage):
    ...
    def create_graph_controls(self):
        ...
        self.group_step_label = ctk.CTkLabel(controls, text="Every ", font=("Inter", 12), text_color="black")
    ...
    def on_interval_change(self, choice):
        if choice == "Session":
            ...
            if choice == "Daily":
                self.group_step_label.configure(text=f"Every {val} day(s)")
            else:
                self.group_step_label.configure(text=f"Every {val} hour(s)")

    def on_group_step_change(self, value):
        v = int(value)
        interval = self.selected_interval.get()
        if interval == "Daily":
            self.group_step_label.configure(text=f"Every {v} day(s)")
        elif interval == "Hourly":
            self.group_step_label.configure(text=f"Every {v} hour(s)")
        else:
            self.group_step_label.configure(text="Grouping by session")
