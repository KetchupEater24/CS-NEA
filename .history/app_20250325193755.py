...
class DecksPage(BasePage):
    # initialises the decks page as a subclass of base page (inheritance)
    ...
    def __init__(self, master, user_id, switch_page):
        ...
        # creates header for decks page --> "My Decks"
        ...
        # filter section (by priority) and search section (to search specific decks)
        self.filter_by_priority = ctk.CTkFrame(self.header, fg_color="transparent")
        self.filter_by_priority.pack(side="right", padx=10)
        # deck_search_input is a variable to hold the deck the user types in the search area
        self.deck_search_input = ctk.StringVar()
        # create the deck search entry field and pack it do display it on the page
        self.deck_search_entry = ctk.CTkEntry(
            ...
            laceholder_text_color="#D1D1D1",
            text_color="#000000", 
            fg_color="white", 
            border_color="#e5e7eb", 
            width=200)
        self.deck_search_entry.pack(side="left", padx=5)
        # deck_priority_input holds the priority the user selects from the dropdown menu
        # default value is "All", to show decks with all types of priority
        self.deck_priority_input = ctk.StringVar(value="All")
        # creates priority dropdown menu and pack it to display on the page
        ...
        # trace_add listens for any changes to deck_search_input and deck_priority_input
        # if the user selects a priority or types something in the search field, the lambda function triggers update_deck_list()
        # to show the now filtered decks
        self.deck_search_input.trace_add("write", lambda *args: self.update_deck_list())
        self.deck_priority_input.trace_add("write", lambda *args: self.update_deck_list())

        # create self.buttons container at top right of decks page and pack it with the add and delete selected self.buttons
        self.self.buttons = ctk.CTkFrame(self.header, fg_color="transparent")
        self.self.buttons.pack(side="right")
        self.ctk.CTkButton(
            self.buttons, 
            text="+ Add", 
            width=70, 
        ...
        self.separator = ctk.CTkFrame(self.main_content, height=1, fg_color="#D1D5DB")
...
class DeckContainer(BaseContainer):
    def __init__(self, master, deck_id, deck_name, card_count, selection_callback, avg_ef, edit_callback, delete_callback):
        ...
        # self.buttons container (Edit/Delete) at bottom (only if callbacks provided)
...
class CardContainer(BaseContainer):
    ...
    def __init__(self, master, card_id, question, answer,
        ...
        # self.buttons container (Edit/Delete)
        self.buttons = ctk.CTkFrame(self.content, fg_color="transparent")
        self.buttons.pack(side="bottom", fill="x")
        btn_container = ctk.CTkFrame(self.buttons, fg_color="transparent")
