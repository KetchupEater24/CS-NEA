...
class DecksPage(BasePage):
    # intiialises decks page as subclass of basepage (inheritance)
    ...
    def __init__(self, master, user_id, switch_page):
        ...
        # header container ("My Decks" title)
    ...
    def update_deck_containers(self):
        ...
        for node in sorted_nodes:
            deck_container = DeckContainer(self.decks_frame,
...
class DeckContainer(BaseContainer):
    # initialises deck container as subclass of base container (inheritance)
    # initialises deck container with deck info (deck id, name, card count, etc) and callbacks for buttons and checkbox selections
    def __init__(self, master, deck_id, deck_name, card_count, selection_callback, avg_ef, edit_callback, delete_callback):
        ...
        # displays deck info on left side of deck container
        ...
        # display how many cards are available for review
        db = Database()
        available_for_review = (
            db.get_available_for_review(self.selection_callback.__self__.user_id, deck_id)
        ...
        # based on avg ef, set priority text
        if self.avg_ef < 2.0:
        ...
        # buttons container (Edit/Delete) at bottom only if the callbacks are not None
        if self.edit_callback is not None and self.delete_callback is not None:
            btn_container = ctk.CTkFrame(self.deck_container, fg_color="transparent")
        ...
        # checkbox at top right of deck container
    ...
    # changes checkbox to purple on toggle
...
class CardsPage(BasePage):
    # initialises cards page as subclass of base page (inheritance)
    # initialises cards page with master, user id, deck id, and switch page function
    def __init__(self, master, user_id, deck_id, switch_page):
        ...
        # variable to hold card priority filter selection (default "All")
        self.card_priority_filter_var = ctk.StringVar(value="All")
        # card priority dropdown: allows filtering cards by priority (all, high, medium, low)
        ...
        # trace changes in search and filter inputs to update displayed cards automatically
        ...
        # delete button: deletes selected cards; initially disabled until a card is selected
        ...
        # add card button: opens dialog to add a new card
        ctk.CTkButton(
            header,
            text="+ Add Card",
            width=70,
        ...
        # cards frame: scrollable area where card containers are displayed
        ...
        # display the cards in the scrollable frame
        self.display_cards()

    # displays cards based on current search and priority filter criteria
    def display_cards(self):
        # clear current card widgets
        for widget in self.cards_frame.winfo_children():
            widget.destroy()

        # get raw cards from the database for the current deck
        ...
        # if no cards match the filters, display a message indicating no cards were found
        if not cards:
        ...
        # create and display a card container for each sorted card
    ...
    # adds a new card by opening an add card dialog
    def add_card(self):
        dialog = AddCardDialog(self, deck_id=self.deck_id)
        dialog.grab_set()

    # edits an existing card by opening an edit card dialog pre-filled with current card info
    ...
    # deletes a card after confirming with the user
    def delete_card(self, card_id):
        if messagebox.askyesno("Delete Card", "Are you sure you want to delete this card?"):
            self.db.delete_card(card_id)
            self.display_cards()

    # toggles the selection state of a card and updates the delete button state accordingly
...
class CardContainer(BaseContainer):
    ...
    # initialises card container with card details and callbacks for edit, delete, and selection actions
    def __init__(self, master, card_id, question, answer, edit_callback, delete_callback, ef, user_id, selection_callback=None):
        super().__init__(master)
        self.card_id = card_id
        self.selection_callback = selection_callback  # callback for tracking selection state
        self.selected = False

        # main container for card content: holds question, answer, and action buttons
        self.content = ctk.CTkFrame(self, fg_color="transparent")
        self.content.pack(fill="x", padx=20, pady=15)

        # question label: displays the card's question in bold text
        ctk.CTkLabel(
            self.content,
            text=question,
            font=("Inter", 16, "bold"),
        ...
        # answer label: displays the card's answer in regular text
        ctk.CTkLabel(
            self.content,
            text=answer,
            font=("Inter", 14),
        ...
        # priority indicator: shows card priority based on easiness factor (ef)
        if ef < 2.0:
            priority_text = "High Priority"
            color = "red"
        elif ef < 2.5:
        ...
        # inner container for buttons, aligned to the right
        btn_container = ctk.CTkFrame(buttons, fg_color="transparent")
        btn_container.pack(side="right", padx=5, pady=5)

        # edit button: calls the edit callback to modify the card
        ctk.CTkButton(
            btn_container,
            text="Edit",
            width=70,
        ...
        # delete button: calls the delete callback to remove the card
        ctk.CTkButton(
            btn_container,
            text="Delete",
            width=70,
        ...
        # multi-selection checkbox at top right: allows for selecting multiple cards
        self.checkbox = ctk.CTkCheckBox(
            self,
            text="",
            width=24,
    ...
    # toggles card selection state when the card container is clicked (avoids direct checkbox clicks)
    def toggle_selection(self, event=None):
    ...
    # changes checkbox appearance based on selection state and calls the selection callback
    def on_checkbox_toggle(self):
        self.selected = self.checkbox.get()
        if self.selection_callback:
            self.selection_callback(self.card_id, self.selected)
