...
class Expression:
    """
    This class encapsulates an infix expression.
    It converts the expression to Reverse Polish Notation (RPN) and evaluates it.
    """
    def __init__(self, expr):
        self.expr = expr
        self.rpn = ConvertToRPN(expr)
        self.value = EvaluateRPN(self.rpn)
...
def PlayGame(Targets, NumbersAllowed, TrainingGame, MaxTarget, MaxNumber):
    ...
    while not GameOver:
        ...
        if CheckIfUserInputValid(UserInput):
            # Use the new Expression class (OOP) to encapsulate conversion and evaluation.
            expression = Expression(UserInput)
            if CheckNumbersUsedAreAllInNumbersAllowed(NumbersAllowed, expression.rpn, MaxNumber):
                IsTarget, Score = CheckIfUserInputEvaluationIsATarget(Targets, expression.rpn, Score)
                if IsTarget:
