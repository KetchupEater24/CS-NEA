...
class DecksPage(BasePage):
    def __init__(self, master, user_id, session, switch_page):
        ...
        super().__init__(master, user_id, session, switch_page)
    ...
    def edit_deck(self, deck_id, current_name):
        ...
        # The EditDeckDialog updates the DB and calls display_decks itself.
...
class DeckContainer(Container):
    def __init__(self, master, deck_id, deck_name, card_count, selection_callback, avg_ef):
        super().__init__(master)
        self.deck_id = deck_id
        self.selection_callback = selection_callback  # For multi-select tracking
        self.selected = False
        self.avg_ef = avg_ef
        ...
        ctk.CTkLabel(info_frame, text=deck_name,
                     font=("Inter", 16, "bold"),
                     text_color="black").pack(anchor="w")
        ctk.CTkLabel(info_frame, text=f"{card_count} cards",
                     font=("Inter", 14),
                     text_color="#6B7280").pack(anchor="w", pady=(5, 0))

        # Display due count (example)
        db = Database()
        due_count = db.get_due_cards_count(self.selection_callback.__self__.user_id, deck_id) if hasattr(
            self.selection_callback, '__self__') else 0
        ctk.CTkLabel(info_frame, text=f"{due_count} available for review",
                     font=("Inter", 12),
                     text_color="#DC2626").pack(anchor="w", pady=(5, 0))
        ...
        if self.avg_ef < 2.0:
            ...
            tag_color = "green"
        ctk.CTkLabel(info_frame, text=priority_text,
                     font=("Inter", 12, "bold"),
                     text_color=tag_color).pack(anchor="w", pady=(5, 0))

        # Right: Checkbox for selection
        self.checkbox = ctk.CTkCheckBox(content,
                                        text="",
                                        width=24,
                                        height=24,
                                        corner_radius=6,
        ...
        content.bind("<Button-1>", self.toggle_selection)
        info_frame.bind("<Button-1>", self.toggle_selection)
    ...
    def on_checkbox_toggle(self):
        ...
        if self.selected:
            ...
            self.checkbox.configure(fg_color="#4f46e5", checkmark_color="white", hover_color="#4f46e5")
        else:
...
class CardContainer(Container):
    def __init__(self, master, card_id, question, answer,
        ...
        ctk.CTkLabel(content, text=question, font=("Inter", 16, "bold"), text_color="black").pack(anchor="w")
        ctk.CTkLabel(content, text=answer, font=("Inter", 14), text_color="black").pack(anchor="w", pady=(5, 0))
        ...
        if ef < 2.0:
            ...
            priority_text = "Low Priority"
            color = "green"
        ctk.CTkLabel(content, text=priority_text, font=("Inter", 12, "bold"), text_color=color).pack(anchor="w",
                                                                                                     pady=(5, 10))
        ...
        ctk.CTkButton(btn_container, text="Edit", width=70, height=32, corner_radius=16,
                      fg_color="#F3F4F6", text_color="black", hover_color="#E5E7EB",
                      command=lambda: edit_callback(card_id)).pack(side="left", padx=(5, 2), pady=5)
        ctk.CTkButton(btn_container, text="Delete", width=70, height=32, corner_radius=16,
                      fg_color="#FEE2E2", text_color="#DC2626", hover_color="#FECACA",
                      command=lambda: delete_callback(card_id)).pack(side="left", padx=(2, 5), pady=5)

        # Multi-selection checkbox at top-right
        self.checkbox = ctk.CTkCheckBox(self, text="", width=24, height=24, corner_radius=6,
                                        command=self.on_checkbox_toggle, hover_color="white")
...
# Updated CardContainer class with priority display based on "ef"
...
class EditDeckDialog(Dialog):
    def __init__(self, parent, deck_id, current_deck_name):
        ...
        self.deck_id = deck_id
        self.parent = parent
        self.create_title_label("Edit Deck")
        ctk.CTkLabel(
            self.container,
        ...
        self.deck_entry = self.create_input_field(current_deck_name)
        self.create_button("Save Changes", self.save_deck)
        self.wait_window()

    def save_deck(self):
        new_name = self.deck_entry.get().strip()
        if not new_name:
            messagebox.showwarning("Warning", "Please enter a deck name")
            return
        db = Database()
        db.update_deck_name(self.deck_id, new_name)
...
class AddCardDialog(Dialog):
    def __init__(self, parent, deck_id):
        super().__init__(title="New Card", width=500, height=800)
        self.deck_id = deck_id
        self.parent = parent
        self.create_title_label("New Card")
...
class AddDeckDialog(Dialog):
    def __init__(self, parent):
        ...
        self.create_title_label("New Deck")
        ctk.CTkLabel(
            self.container,
...
# Assume Sidebar, QuizSession are defined/imported elsewhere.
...
class QuizPage(BasePage):
    def __init__(self, master, user_id, session, switch_page):
        ...
        self.deck_search_var = ctk.StringVar()
        self.deck_search_entry = ctk.CTkEntry(controls_frame, textvariable=self.deck_search_var,
                                              placeholder_text="Search deck", placeholder_text_color="#D1D1D1",
                                              text_color="#000000", fg_color="white", border_color="#e5e7eb", width=150)
        self.deck_search_entry.pack(side="left", padx=(0, 20))

        self.deck_priority_filter_var = ctk.StringVar(value="All")
        self.deck_priority_filter_menu = ctk.CTkOptionMenu(controls_frame,
                                                           values=["All", "High", "Medium", "Low"],
                                                           variable=self.deck_priority_filter_var,
                                                           width=120, fg_color="white", button_color="#F3F4F6",
        ...
        self.start_button = ctk.CTkButton(controls_frame, text="Start Quiz", width=120, height=32,
                                          corner_radius=16, fg_color="#4F46E5", hover_color="#4338CA",
                                          state="disabled", command=self.start_quiz)
        ...
        ctk.CTkLabel(self.selection_frame, text="Select decks to quiz yourself on",
                     font=("Inter", 16, "bold"), text_color="black").pack(anchor="w", pady=(0, 10))

        self.decks_frame = ctk.CTkScrollableFrame(self.selection_frame, fg_color="transparent",
                                                  scrollbar_button_color="#E5E7EB",
                                                  scrollbar_button_hover_color="#D1D5DB")
    ...
    def display_decks(self):
        ...
        for node in sorted_nodes:
            ...
            if col == 3:
                col = 0
                row += 1
        self.start_button.configure(state="normal" if self.selected_decks else "disabled")
    ...
    def start_quiz(self):
        ...
        for widget in self.master.winfo_children():
            widget.destroy()
        QuizSession(self.master, self.user_id, list(self.selected_decks)[0], list(self.selected_decks), self.session,
                    self.switch_page)
# Example QuizDeckContainer with priority tag (similar to DeckContainer)
...
class QuizSession(ctk.CTkFrame):
    ...
    def setup_ui(self):
        ...
        for label, quality, color in options:
    ...
    def show_summary(self, accuracy, avg_time):
        ...
        ctk.CTkButton(
            ...
            text_color="black",
            hover_color="#E5E7EB",
            command=lambda: self.switch_page(QuizPage, user_id=self.user_id, session=self.session,
...
class AnalyticsPage(BasePage):
    def __init__(self, master, user_id, session, switch_page):
        ...
        self.content_container = ctk.CTkScrollableFrame(self.main_content, fg_color="white",
    ...
    def create_stats_section(self):
        stats_container = ctk.CTkFrame(self.content_container, fg_color="white",
                                       corner_radius=8, border_width=0, border_color="#E5E7EB")
        stats_container.pack(fill="x", pady=(0, 20))
        ctk.CTkLabel(stats_container, text="Overall Statistics", font=("Inter", 18, "bold"),
                     text_color="#111827").pack(anchor="w", padx=20, pady=(15, 10))
        stats_grid = ctk.CTkFrame(stats_container, fg_color="white")
        ...
        for i in range(0, len(stat_items), 3):
            row_frame = ctk.CTkFrame(stats_grid, fg_color="white")
            row_frame.pack(fill="x", pady=5)
            for j in range(3):
                if i + j < len(stat_items):
                    label_text, value, icon = stat_items[i + j]
                    self.create_stat_card(row_frame, label_text, value, icon)

    def create_stat_card(self, parent, label_text, value, icon):
        stat_card = ctk.CTkFrame(parent, fg_color="white", corner_radius=8,
                                 border_width=1, border_color="#E5E7EB", width=180, height=100)
        stat_card.pack(side="left", padx=5, pady=5, fill="both", expand=True)
        header_frame = ctk.CTkFrame(stat_card, fg_color="white")
        header_frame.pack(fill="x", padx=10, pady=(10, 5))
        ctk.CTkLabel(header_frame, text=icon, font=("Inter", 16), text_color="#4B5563").pack(side="left")
        ctk.CTkLabel(header_frame, text=label_text, font=("Inter", 12), text_color="#4B5563").pack(side="left",
                                                                                                   padx=(5, 0))
        ctk.CTkLabel(stat_card, text=value, font=("Inter", 22, "bold"), text_color="#111827").pack(fill="both",
                                                                                                   expand=True)
    ...
    def create_deck_performance_section(self):
        ...
        if not decks:
            return
        perf_container = ctk.CTkFrame(self.content_container, fg_color="white",
                                      corner_radius=8, border_width=0, border_color="#E5E7EB")
        perf_container.pack(fill="x", pady=(0, 20))
        ctk.CTkLabel(perf_container, text="Deck Performance Scores", font=("Inter", 18, "bold"),
                     text_color="#111827").pack(anchor="w", padx=20, pady=(15, 10))
    ...
    def create_deck_score_card(self, parent, deck_id, deck_name, score):
        ...
        score_card = ctk.CTkFrame(deck_container, fg_color="white", corner_radius=8,
                                  border_width=1, border_color="#E5E7EB")
        ...
        ctk.CTkLabel(name_frame, text=deck_name, font=("Inter", 14, "bold"), text_color="#111827").pack(anchor="w")
        ...
        if score < 50:
            ...
            color = "#10B981"
        ctk.CTkLabel(right_frame, text=f"{score:.1f}/100", font=("Inter", 14, "bold"), text_color=color).pack(
            side="right", padx=(10, 0))
        view_stats_button = ctk.CTkButton(right_frame, text="View Details", width=100, height=32,
                                          corner_radius=16, fg_color="#F3F4F6", text_color="black",
                                          hover_color="#E5E7EB", command=lambda: None)
        view_stats_button.pack(side="right", padx=(10, 0))

    def create_graph_controls(self):
        graph_container = ctk.CTkFrame(self.content_container, fg_color="white", corner_radius=8,
                                       border_width=0, border_color="#E5E7EB")
        graph_container.pack(fill="both", expand=True, pady=(0, 20))
        ctk.CTkLabel(graph_container, text="Performance Graphs", font=("Inter", 18, "bold"),
        ...
        ctk.CTkLabel(controls_row, text="Select Deck", font=("Inter", 14, "bold"),
                     text_color="#111827").pack(side="left", padx=(0, 5))
        decks = self.db.get_decks(self.user_id)
        self.deck_options = {deck_name: str(deck_id) for deck_id, deck_name in decks}
        self.deck_id_to_name = {str(deck_id): deck_name for deck_id, deck_name in decks}
        ...
        self.deck_menu = ctk.CTkOptionMenu(controls_row, values=list(self.deck_options.keys()),
                                           variable=self.selected_deck_name, width=140, corner_radius=8,
                                           dropdown_font=("Inter", 12), font=("Inter", 12),
                                           fg_color="white", button_color="#F3F4F6",
                                           button_hover_color="#E5E7EB", text_color="#111827",
                                           command=lambda x: None)
        self.deck_menu.pack(side="left", padx=(0, 20))
        ctk.CTkLabel(controls_row, text="Select Graph Type", font=("Inter", 14, "bold"),
                     text_color="#111827").pack(side="left", padx=(0, 5))
        self.selected_graph = ctk.StringVar(value="Accuracy Over Time")
        self.graph_menu = ctk.CTkOptionMenu(controls_row,
        ...
        show_graph_btn = ctk.CTkButton(controls_row, text="Show Graph", width=100, height=32,
                                       corner_radius=16, font=("Inter", 14),
    ...
    def create_return_button(self):
        ...
        ctk.CTkButton(button_container, text="Return to Dashboard", width=200, height=40,
                      corner_radius=16, font=("Inter", 14), fg_color="#F3F4F6", text_color="black",
                      hover_color="#E5E7EB", command=lambda: self.switch_page(DecksPage, user_id=self.user_id,
